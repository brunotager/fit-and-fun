...
export default function WorkoutsPage() {
    ...
    const { profile, progress, userId, workoutPlan, isPlanLoading, updateProfile } = useFitFun();
        ...
        return (
            <div className="flex-1 flex flex-col bg-[#FFFDF7] animate-in fade-in duration-700 h-full">
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                    ...
                    <h1 className="text-3xl font-black text-gray-900 uppercase tracking-tight mb-4">Level 1 Complete!</h1>
                    <p className="text-gray-600 mb-8 max-w-sm font-medium">
                        Congratulations! You have completed Level 1 of your Fit & Fun journey. 
                        You've built a solid habit of 5 minutes of exercise every day! Next Level (Level 2: 10-minute daily workouts) is coming soon in our 28-Day Plan!
                    </p>
                    <button 
                        onClick={() => {
                        ...
                        className="w-full max-w-xs bg-brand-500 hover:bg-brand-600 text-white font-bold py-4 rounded-[20px] shadow-[0_8px_30px_rgb(232,106,32,0.25)] active:scale-95 transition-transform uppercase tracking-wider text-sm"
                    >
                        Want More Workouts?
                    </button>
                    <button 
                ...
                {showInterestModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                        <div className="bg-white rounded-[20px] w-full max-w-sm p-8 shadow-2xl relative text-center">
                            {!isEmailSubmitted ? (
                                <>
                                    ...
                                    <p className="text-sm text-stone-600 font-semibold leading-relaxed mb-6">
                                        We are actively designing the extended Level 2 and beyond. Join the waitlist and you'll be the first to know when the next levels drop!
                                    </p>
                                    ...
                                    <div className="flex flex-col gap-3 mb-6">
                                        <input 
                                            ...
                                            className="w-full bg-stone-50 border border-stone-200 rounded-[20px] p-4 text-stone-700 font-medium outline-none focus:border-brand-500 transition-colors placeholder:text-stone-400 text-center"
                                        />
                                        <button 
                                            onClick={async () => {
                                                ...
                                                try {
                                                    ...
                                                    if (res.ok) {
                                                        updateProfile({ waitlistEmail: email });
                                            ...
                                            className="w-full bg-brand-500 hover:bg-brand-600 text-white font-bold py-4 rounded-[20px] transition-colors active:scale-95 text-lg shadow-[0_8px_30px_rgb(232,106,32,0.25)] disabled:opacity-50"
                                            disabled={!email}
                                        >
                                            Join Waitlist
                                        </button>
                                ...
                                <div className="animate-in zoom-in-95 duration-500 py-4">
                                    ...
                                    <button 
                                        onClick={() => setShowInterestModal(false)}
                                        className="w-full bg-stone-100 hover:bg-stone-200 text-stone-700 font-bold py-4 rounded-[20px] transition-colors active:scale-95 text-lg"
                                    >
                                        Awesome
                                    </button>
                                </div>
    ...
    if (isPlanLoading) {
        return (
            <div className="flex-1 flex flex-col bg-[#FFFDF7] animate-in fade-in duration-700 h-full">
                <Header title="Level 1 Journey" />
                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                    <div className="w-12 h-12 border-4 border-brand-500 border-t-transparent rounded-full animate-spin mb-4"></div>
                    <h2 className="text-xl font-bold text-gray-900 mb-2">Assembling Your Journey...</h2>
                    <p className="text-gray-500 text-sm max-w-xs leading-relaxed font-medium">
                        Coach Gabi is preparing your workout. Get ready to move!
                    </p>
                </div>
            </div>
        );
    ...
    const plan = workoutPlan.length > 0 ? workoutPlan : generate7DayPlan();
            ...
            <div className="flex-1 flex flex-col bg-[#FFFDF7] animate-in fade-in duration-700 h-full">
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                    ...
                    <button
                        ...
                        className="bg-brand-500 hover:bg-brand-600 text-white font-bold py-3 px-8 rounded-[20px] active:scale-95 transition-transform"
                    >
                        Refresh
                    </button>
                </div>
        ...
        <div className="flex-1 flex flex-col bg-[#FFFDF7] overflow-y-auto custom-scrollbar animate-in fade-in duration-700 pb-10 min-h-0">
            ...
            <div className="flex-1 flex flex-col px-6 mt-6 relative">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="bg-stone-100 text-stone-500 px-4 py-1.5 rounded-full font-bold text-sm">
                        {currentDay} of 7
                    </div>
                </div>

                {/* Workout Card */}
                <div className="bg-white rounded-[20px] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-stone-100 relative overflow-hidden flex flex-col items-center text-center">
                    {/* Decorative Blob */}
                    ...
                    <button
                        ...
                        className="w-full bg-brand-500 hover:bg-brand-600 text-white font-black py-4 rounded-[20px] shadow-[0_8px_30px_rgb(232,106,32,0.25)] active:scale-95 transition-transform text-lg uppercase tracking-wide z-10"
                    >
                        START WORKOUT
                    </button>
                </div>
